...
const BarChart = ({ data }) => {
  ...
  const barW = 28; // slim fixed width
  const yTicks = 4;
...
const ProjectDetailsPage = ({ projectId }) => {
  ...
  useEffect(() => {
    const go = async () => {
      ...
      try {
        ...
        const projRes = await fetch(`https://security-audit-accelerator-backend-196053730058.asia-south1.run.app/api/projects/${projectId}`, { headers: h });
        if (!projRes.ok) throw new Error('Not found');
        const projData = await projRes.json();
        setProject(projData);

        const scanRes  = await fetch(`https://security-audit-accelerator-backend-196053730058.asia-south1.run.app/api/projects/all/scans`, { headers: h });
  ...
  const goToReport = (scan) => {
    const d = { score: scan.score, vulnerabilities: scan.findings || [], scanned: scan.scannedResources, provider: project?.provider || 'gcp', isHistory: true };
    localStorage.setItem('last_viewed_scan', JSON.stringify(d));
  ...
  const medium   = latest?.mediumCount   || 0;
  const totalV   = critical + high + medium;
    ...
    <>
      ...
      <div style={{ paddingBottom: 32 }}>
        ...
        <div style={{
          ...
          {latest && (
            <button className="pd-btn" onClick={() => goToReport(latest)} style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: 'var(--color-primary)', color: '#000', fontWeight: 700, cursor: 'pointer', fontSize: 12 }}>
              View Full Report →
            </button>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
          <div className="pd-card" style={{ flex: 1, minWidth: 120 }}>
            <StatCard label="Total Scans"    value={scans.length}                         icon="🔍" accent="#6366f1" />
          </div>
          <div className="pd-card" style={{ flex: 1, minWidth: 120 }}>
            <StatCard label="Latest Score"   value={latest ? `${latest.score}%` : '—'}    icon="🛡️" accent={scoreColor(latest?.score)} />
          </div>
          <div className="pd-card" style={{ flex: 1, minWidth: 120 }}>
            <StatCard label="Resources"      value={latest?.scannedResources ?? '—'}       icon="🖥️" accent="#10b981" />
          </div>
          <div className="pd-card" style={{ flex: 1, minWidth: 120 }}>
            <StatCard label="Total Issues"   value={totalV.toLocaleString()}               icon="⚠️" accent={totalV > 0 ? '#ef4444' : '#22c55e'} />
          </div>
        </div>
        ...
        {latest && (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--color-text-muted)' }}>Vulnerability Breakdown</span>
              <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                ...
                <button className="pd-btn" onClick={() => goToReport(latest)} style={{ padding: '5px 14px', borderRadius: 6, border: 'none', background: 'var(--color-primary)', color: '#000', fontWeight: 700, cursor: 'pointer', fontSize: 11 }}>
                  View Report →
                </button>
              </div>
            </div>
