...
const sendAuditEmail = async (userEmail, scanData, projectName) => {
    ...
    try {
        ...
        const htmlContent = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
                    ...
                    <p style="color: rgba(255,255,255,0.8); margin: 8px 0 0 0;">CA AuditScope — Scheduled Report</p>
                </div>
                <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e9ecef;">
                    ...
                    <p style="color: #6c757d; font-size: 14px;">
                        The full PDF audit report is attached to this email. Log in to your CA AuditScope dashboard for detailed remediation steps.
                    </p>
                    <p style="color: #adb5bd; font-size: 12px; margin-top: 20px;">
                        This is an automated report generated by CA AuditScope at ${new Date().toLocaleString()}
                    </p>
                </div>
            </div>
        `;
        ...
        const filename = `CA_AuditScope_Report_${projectName.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.pdf`;

        await transporter.sendMail({
            from: `"CA AuditScope Automation" <${process.env.SMTP_USER}>`,
            to: userEmail,
